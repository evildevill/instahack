...
# 1. Added commented docs + made the code structure more clean as well as now it is good for coders to read the actual code base
# 2. Removed some older errors and updated the older version of the code to the newer and way that uses less number of lines of the code + added comments to each part of the script such that the programmers and hackers will understand the code even if they don't touch the source files for a while
# 3. Adding more colors and making the proper use of the ANSII color code variables + using more bright colors for better console look of the tool.
# 4. Removing the lolcat utility dependency, instead we are using plain colors to print it on the console screen. This makes the changes of error much more often. Like why installing another software for just printing a banner, as well as the software also requires ruby, ruby-environment and ruby-interpreter to be installed on the computer system. Thus, too many dependencies are there for this just one banner printing. So, we removed it.
#
...
function checkUpdates() {
	...
	if wget --spider https://raw.githubusercontent.com/evildevill/instahack/master/update.v1.3 2>/dev/null; then
		...
		# ----
		# We donot do that shit here, because we dont want to create a waste file for just marking purpose. Lets directly call the update.sh script
		# ----
		if [[ -f "bash $HOME/instahack/core/update.sh" ]]; then
			...
			# If the update.sh script does not exists in the core/ of the folder of this project, then we display the error message on the console screen
...
echo -e "\e[1;92m[\e[0m\e[1;77m8\e[0m\e[1;92m]\e[0m\e[1;93m Exit\e[0m\n"
